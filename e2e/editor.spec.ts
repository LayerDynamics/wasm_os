...
test("code-only Backspace still reaches the editor", async ({ page }) => {
  await ready(page);
  ...
  await page.keyboard.type("abc", { delay: 30 });
  await page.waitForTimeout(120);
  await page.evaluate(() => window.dispatchEvent(new KeyboardEvent("keydown", { code: "Backspace", key: "" })));
  await page.waitForTimeout(120);
  await page.keyboard.press("Control+KeyS");
  await page.waitForTimeout(400);
  ...
  expect(text).toBe("abSEED");
});

test("Delete (forward) removes the character under the cursor in the editor", async ({ page }) => {
  // Regression: the editor handled Backspace but not Delete (KEY_DELETE 0x108), so
  // forward-delete did nothing. The editor loads with the cursor at the start of the
  // document, so Delete must erase the first character.
  await ready(page);
  ...
  // Cursor is at the document start; Delete erases the first char ("S"), then save.
  ...
  expect(text).toBe("EED"); // the leading "S" was forward-deleted
});

test("Home, End, and Tab reach the editor through brokered input", async ({ page }) => {
  await ready(page);
  ...
  // Type before the seed, jump to both line ends, then insert the editor's
  // four-space Tab expansion at the end of the line.
  await page.keyboard.type("abc", { delay: 30 });
