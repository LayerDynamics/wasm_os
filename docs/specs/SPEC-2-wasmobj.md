# SPEC-2: wasmobj — the wasm-object document container
...
This spec defines a **userland library**, not a kernel subsystem. It deliberately does **not** touch the existing host TypeScript `Blockstore` (`packages/host/src/blockstore/`), which is a kernel-imported, sync-over-async OPFS/IndexedDB key/value store at a different layer. `wasmobj` runs *inside a `wasm32-wasi` guest process*, reaches files only through WASI syscalls, and produces/consumes ordinary VFS files. (The crate was renamed from `byteblockstorage` to `wasmobj` precisely so its name no longer reads like the host `Blockstore` — see OQ-2.)
...
| FR-3 | MUST | The object MUST embed a self-describing header in a **custom section** named `wob0`, recording: magic, format version, the writable window’s **byte offset within the file**, its `capacity`, the current `content_len`, and a `content_type` tag. |
| FR-4 | MUST | Given any object this library recognizes, the library MUST locate the writable window via the `wob0` header and read back exactly `content_len` content bytes (excluding placeholder padding). |
| FR-5 | MUST | On save where `content_len ≤ capacity`, the library MUST overwrite the window **in place with no module re-encode**: write the content bytes, pad the remainder with `0x20`, and update `content_len` in the `wob0` header. |
...
| FR-NG-3 | WONT | Parse, relocate, or rewrite arbitrary third-party `.wasm` modules — only objects bearing a valid `wob0` header are read/written. |
...
| Object overhead beyond the window | < 1 KiB (module preamble + `wob0` header) | `len(object) − capacity` for an empty object at each tier |
...
| Malformed-input safety | Feeding non-`wob0` or corrupted bytes returns a typed `Err`, never panics/traps the guest (fuzz/`Result` test). |
...
- **Forgery resistance:** the `wob0` header is validated before any in-place write (FR-11); an object whose declared window offset/capacity does not match its actual data segment is rejected, so a crafted object cannot steer a write outside the window.
...
- The format is forward-versioned (`wob0` → `bbs1`…) so larger tiers or chained blocks (OQ-3) can be added without breaking readers of older objects.
...
3. **Metadata lives only in a custom section.** Per the WebAssembly spec, custom sections are the sole standard place for non-executable metadata and are ignored by engines — so the `wob0` header (FR-3) cannot disturb module validity or execution.
...
6. **Workspace registration.** `crates/wasmobj` must be added to the root `Cargo.toml` `members` and gain a `Cargo.toml` using the workspace `edition`/`version`/`license` (matching sibling crates like `catfile`).
...
┌───────────────────────────────────────┐     ┌─────────────────────────┐
│ editor app  ── links ──▶ wasmobj│   │ /home/templates/blank.wasm│
...
#### Component: `format`
- **Responsibility:** Define the on-disk wasm-object layout and the `wob0` header; encode/decode the header; enumerate block tiers.
...
#### Component: `mint`
- **Responsibility:** Emit a fresh, valid module at a given tier — module preamble, a memory, one active data segment of `capacity` × `0x20`, the `wob0` custom section pointing at that segment’s file offset, and the `render` export (FR-9, MUST).
...
- **Technology:** Rust (`crates/apps/editor`), linking `wasmobj`.
- **Interfaces:** internal to the editor’s open/save handlers.
- **Dependencies:** `wasmobj`.
...
│ ── CUSTOM SECTION name="wob0" (the name IS the magic) ────────  │
│   Header (14 bytes) { version:u8=1,                              │
...
- **`wob0` header** — created at mint; `content_len` patched on every save; `capacity`/`window_offset` change only on re-pack.
...
- A `wobj info` (FR-13) / `verify` path prints the decoded `wob0` header (version, offset, capacity, content_len, type) for inspection.
...
- Docs: add a short `crates/wasmobj/README.md` replacement describing the format and the `wob0` header (the current one-liner is the seed, not the doc).
...
| Term | Meaning |
|------|---------|
| **wasm object** | A standard `.wasm` module used as a document container: its active data segment holds the content; a `wob0` custom section describes it. |
...
| **`wob0` header** | The custom-section metadata: magic, version, window offset, capacity, content_len, content_type. |
...
### Appendix B — `wob0` Header Layout (V1)

The header is the 14-byte payload of a WebAssembly custom section whose **name
`wob0` is the magic** (there is no separate magic field). Offsets are within the
section payload:

```text
offset  size  field          notes
0       1     version        = 1
1       4     window_offset  u32 LE — file offset of first window byte
5       4     capacity       u32 LE — window length in bytes (== data segment len)
9       4     content_len    u32 LE — valid content bytes (rest is 0x20 padding)
13      1     content_type   0 = text/plain, 1 = application/octet-stream (FR-15)
```
