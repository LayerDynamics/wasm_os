...
| 5 | `npm run verify` green incl. M0 tri-backend persistence regression through the async proxy | local `npm run verify` (exit 0); `e2e/boot.spec.ts` | ✅ PASS |
...
playwright   : 9 passed — boot<1.5s+tierA+init-proc, tri-backend persist, fsDelete,
               OpfsBlockstore real-OPFS, IdbBlockstore real-IDB (M0 regression);
               hello spawn/stdout/exit0, two-proc isolation, crash containment,
               catfile path_open+fd_read (M1)
...
1. **Generated bindings are gitignored build output** (since commit `5d735e3`),
   NOT committed. The plan's "commit `packages/abi/generated`" + "`npm run drift`"
   steps reflected the *older* M0-STATUS design; the real gate is `npm run build`
   (regenerate) + typecheck + E2E exercising fresh bindings. No bindings were
   committed.
...
4. **`fd_readdir` synthesized from the flat-key VFS listing** — provisional until
   M2 introduces a real hierarchical directory tree.
...
## Deferred to M2+ (genuinely out of M1 scope; tracked)
