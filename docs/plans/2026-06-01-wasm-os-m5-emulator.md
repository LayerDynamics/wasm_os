# M5 — Emulator as a Privileged Process ("Linux in a tab")
...
**Status:** PLAN (authored 2026-06-01). Builds on M0–M4 (all merged to `main`;
M4 = `docs/M4-STATUS.md`). Source of truth: `docs/specs/SPEC-1-wasm-os.md`.

---

## ⚠️ Two decisions to confirm before execution (explicitly vetoable)

These two are load-bearing and were chosen from a free-text / "maximal" answer —
veto either at review and the plan adjusts.

1. **Emulator core = v86 (x86).** You answered "whatever is the **most capable**,
   but the system must be able to **run the image from within it**." We read that as:
   - **"most capable" → v86** — a mature x86 emulator with a WASM core, the proven
     "boot Linux in a browser tab" path. It boots full x86 Linux (Buildroot/Alpine/
     BusyBox) **and arbitrary x86 disk/ISO images** (FreeDOS, KolibriOS, ReactOS…),
     which is the broadest "run any image" capability.
   - **"run the image from within it" → the `net_request` broker + image-load path**
     (Phase B): WASM_OS fetches/loads an image at runtime and boots it internally.
   - **The fork, if we misread you:** if you meant "run **container** workloads,"
     that is **container2wasm**, and Phases A/B change shape (OCI image plumbing,
     heavier assets). Say so and we re-scope.
2. **Licensing — v86 is GPLv2.** Bundling/serving v86 has copyleft implications for
   how WASM_OS is distributed. You picked "most capable"; this surfaces the trade so
   it's a *decision*, not a surprise. Alternatives if GPL is unacceptable: a
   permissively-licensed RISC-V core (TinyEMU, MIT) — less turnkey for a full distro.
   Does **not** block planning; flag at review.

---

## What M5 is (and is NOT)

M5 is an **integration milestone**, not a kernel-construction milestone. We are not
writing a CPU emulator — that is infeasible and misses the point. We integrate a
**third-party WASM CPU emulator (v86)** as a first-class WASM_OS **process**: it
...
capability `net_request`, deferred since M3, **resolved here**), **FR-23** (canvas
framebuffer surface), and **FR-36** (all new ABI through the Binder).

**Confirmed scope decisions (planning Q&A): defer nothing.**
1. **Core = v86 (x86)**, integrated as a privileged process (see veto note above).
...
3. **FR-29 shared folder: included** — a host `/home` subtree bridged into guest
   Linux via v86's virtio-9p, backed by the WASM_OS VFS.
4. **Dev practice:** per-task commit (+ its tests) → one branch → PR → drive CI green
   → you merge. Identical to M1–M4. **Tier A only** (SAB), consistent with M1–M4.

---

## The one structural rule for this milestone: **spike first, assert on serial**

Unlike M1–M4, the "spine" is **not our kernel** — it is *"does the third-party core
even work in our headless environment, and how do we assert on it?"* So:

- **Task 1 is a time-boxed v86 spike** that empirically answers: can v86 boot a tiny
  BusyBox `bzImage` to a shell **in headless Playwright**, and **how do we assert**?
...
- **Assert on serial-console TEXT, never framebuffer pixels.** Boot the guest with
  `console=ttyS0`, capture v86's `serial0-output-*` events, and assert on captured
...
- **Serial boot (early, easy assert) precedes graphical framebuffer (polish).** We
  prove "Linux boots to a shell" over serial **before** wiring v86's screen into a
  `win_surface` SAB. Likewise input: prefer v86 `keyboard_send_text()` before
  synthesizing PS/2 scancodes.
...
- **A new process *kind*: `Native`/privileged (non-ring).** Today every process is a
  ...
  **makes no WASI syscalls** — it runs its own CPU loop in a dedicated worker. The
  ...
  is killable, but is **not** pumped by `service_syscall`. `reap` (M4) already tells
...
- **Dedicated emulator worker** (`packages/host/src/worker/emulator-worker.ts`):
  hosts v86 (`libv86` + `v86.wasm` + SeaBIOS/VGABIOS), owns the boot loop, exposes
...
- **Run-to-budget scheduling (FR-28).** The emulator runs continuously in its own
  ...
  than reporting 0).
- **Framebuffer present reuses M3.** v86's screen adapter renders into a compositor
  **canvas surface** (`win_surface`/`win_present` path) — the VGA/console framebuffer
  is presented in a real window. Input reuses the M3 broker (focused window →
  `deliverInput` → emulator worker → v86 keyboard/mouse).
...
- **virtio-9p shared folder (FR-29).** v86's 9p filesystem is backed by a JS adapter
...
- **Guest-disk persistence (FR-35 tie-in).** The guest's disk/state image is
  ...
  taskbar launcher + the M4 session manifest.
...
## M5 exit criteria (definition of done — spec §4.1 Phase 5 / §5)

**Phase A criteria are the spec's M5 MUSTs and are independently shippable.**
...
8. **Guest session persists across reload** (FR-35 tie-in) and "Linux" is launchable
   from the taskbar.
9. `npm run verify` is **green**, including the **M0–M4 regression suite**; the heavy
...
- **Writing an emulator core** — we integrate v86; no from-scratch CPU.
...
- **Multiple concurrent emulator instances** — one privileged emulator process is the
  M5 target (the design does not preclude more later).

---

## Phase A — Boots to a shell (the M5 MUSTs; independently shippable)

### T1 — v86 boot **spike**: BusyBox → shell, serial-asserted in Playwright (time-boxed)
**The empirical foundation. Author T2+ from what this resolves.**
- Vendor v86: add the `v86` dependency (or vendor `libv86.js` + `v86.wasm` +
  `seabios.bin` + `vgabios.bin`) under `third_party/v86/` (note GPLv2 in the file +
  `M5-STATUS`). Obtain the **smallest** real Linux: a BusyBox/Buildroot `bzImage`
  (+ tiny initramfs) — target a few MB; place under `assets/linux/` served by the
  dev server.
- A minimal harness page (or reuse the host) instantiates v86 in a **dedicated
  worker**, boots with `cmdline: "console=ttyS0 ..."`, and pipes `serial0-output-*`
...
- **Test (the deliverable):** a headless Playwright spec boots the image and asserts
  ...
  timeout. Records, in `M5-STATUS` notes: boot time, image size, the exact v86 API
  shape (events, options) the rest of M5 builds on.
- **Resolve before T2:** worker-vs-main, `serial0-output` event shape,
  `keyboard_send_text` availability, OffscreenCanvas vs SAB for the screen.
...
- Kernel: a process **kind** (`Native`/privileged) — PID, capability set, `proc_list`
  entry, **not** pumped by the WASI ring; reuse M4 `reap` for kill. Decide + wire
...
- Route the focused emulator window's input (M3 `deliverInput`) into the worker →
  v86 (`keyboard_send_text` first; PS/2 scancodes only if required).
...
- Wire v86's screen adapter into a compositor **canvas surface** (`win_surface`/
  `win_present`), so the VGA/console renders in a window alongside serial.
...
- **E2E (the exit-criteria scenario):** Linux booted + a concurrent WASM_OS process
  ...
  peer + desktop survive. ← **Phase A = M5 MUST met; tag a shippable point.**
...
- **E2E (slow lane):** fetch a locally-served `bzImage` via the broker and boot it to
  a serial shell.
...
- Back v86's 9p filesystem with a JS adapter over `control.fsRead/fsWrite/fsList` for
...
- Persist the guest disk/state to OPFS so a booted session survives reload; add
  "Linux" to the taskbar launcher and the M4 session manifest (re-open on reload).
- **E2E:** create a file in the guest → reload → it's still there (or the disk image
  persisted), and "Linux" re-opens from the session.

### T10 — Full M5 E2E + **slow-lane CI** + kernel/Binder tests + `M5-STATUS` + verify + PR
- A separate **slow CI lane** for the boot/serial tests (generous timeouts, asset
  ...
  lane keeps M0–M4 + the light M5 checks.
- `docs/M5-STATUS.md` (exit-criteria table, verify-gate breakdown, as-built
  deviations, licensing note); update the README L5 row.
...
| Risk | Mitigation |
|------|------------|
| **v86 doesn't behave headlessly / asserting is hard** | T1 spike resolves this *first*; assert on **serial text**, not pixels. If v86 can't serial-boot headlessly, escalate the core choice (RISC-V/TinyEMU) before building T2+. |
| **CI weight + flakiness** (multi-MB image, multi-second boot) | Smallest BusyBox `bzImage`; **separate slow lane**; generous timeouts; assets served locally (no remote fetch in the fast lane). |
| **GPLv2 contamination** (v86) | Surfaced as an explicit decision above; isolate v86 under `third_party/` with its license; reconsider core if unacceptable. |
...
| **Emulator can't be reaped (non-ring worker)** | M4 `reap` already just tells the host to terminate a worker; T2 verifies it on the native kind. |
