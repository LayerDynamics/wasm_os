...
- **`wasmos:kernel` guest stubs are checked by Binder** against `wit/kernel/kernel.wit` (FR-36), including parameter and return types; the process-worker shim provides the matching host-side imports.
...
**Files:** `wit/kernel/kernel.wit`; `tools/binder` (generate component bindings and check Rust guest stubs); `crates/wasmos-sys` (hand-written transport shim checked against WIT); `packages/host/src/worker/wasi-shim.ts` (+ `wasmos_kernel` import namespace); `kernel-worker.ts` (spawn choreography); `syscall.rs` (SPAWN/PIPE/WAIT/DUP2 opcodes); tests.

**Step 1 — `wit/kernel/kernel.wit`:** the guest process-control surface — `spawn(path, argv, stdio, cwd, capability grants)` → pid, `pipe()` → `(read-fd, write-fd, pipe-id)`, `wait(pid)` → exit-code (parks). `stdio` lets the shell set a child's fd0/1/2 to a pipe end, a file, or the terminal.

**Step 2 — Binder:** `binder gen` generates the tracked TypeScript component bindings. `binder kernel-check` parses `wit/kernel/kernel.wit` and checks the hand-written `crates/wasmos-sys` transport shim's function names, parameter counts, parameter types, and return types. `binder check` runs both gates (FR-36).
