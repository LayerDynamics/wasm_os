...
A document container that **is a WebAssembly module**. A `byteblockstorage`
...
This is the userland (guest) library for [SPEC-2](../../docs/specs/SPEC-2-byteblockstorage.md).
...
A standard wasm module with two byteblockstorage additions:
...
The **`bbs0`** custom section is the self-describing header (the section name is
the magic):
...
in `bbs0` (for external readers). Because the data segment's length is fixed per
...
```rust
use byteblockstorage::{mint, read, save, verify, write_in_place, repack, extract, Tier};
...
- `cargo test -p byteblockstorage` — unit + `proptest` round-trip/validity
  (`wasmparser`) + a `wasmtime` proof that an object self-executes (FR-9).
- `e2e/byteblockstorage.spec.ts` — full browser flow: nano creates a wasm object,
